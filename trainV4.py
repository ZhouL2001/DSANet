...
from netV4 import my_model
import segmentation_models_pytorch as smp
from dataset import TrainDataset, TestDataset
from utils import clip_gradient, adjust_lr, ModelEma, init_distributed_mode, get_rank
from losses import bce_dice_mae_loss, dice_loss
import losses

struct_loss = losses.structure_loss().cuda()
...
def train(args, train_loader, val_loader, model, optimizer, epoch, total_step, save_path, ema, scalar, writer):
    ...
    for i, (images, targets, bodygt) in enumerate(train_loader, start=1):
        ...
        # pred = model(images, targets[:, 0], False)
        # pred = model(images)
        pred, p1, p2 = model(images)
        
        pred  = F.interpolate(pred, size=targets.shape[3:], mode='bilinear', align_corners=True)
        p1    = F.interpolate(p1, size=targets.shape[3:], mode='bilinear', align_corners=True)
        p2    = F.interpolate(p2, size=targets.shape[3:], mode='bilinear', align_corners=True)
        ...
        if scalar:
            with autocast():
                seg_loss = dice_loss(pred, targets[:,0]) + struct_loss(pred, targets[:,0])
                body_loss = dice_loss(p1, targets[:, 1]) + struct_loss(p1, targets[:, 1]) + dice_loss(p2, targets[:, 2]) + struct_loss(p2, targets[:, 2])
                # seg_loss  = bce_dice_mae_loss(pred, targets[:,0])
                # body_loss  = bce_dice_mae_loss(p1, targets[:,1]) + bce_dice_mae_loss(p2, targets[:,2])
                # body_loss = bce_dice_mae_loss(b1, targets[:,1]) + bce_dice_mae_loss(b2, targets[:,2])
                # body_loss = 0.0
                # theta     = 0.3
                all_loss  = seg_loss + body_loss
        ...
        if i % 50 == 0 or i == len(train_loader) or i==1:
            print(f'TRAIN: Epoch [{epoch:03d}/{args.epoch:03d}], Step [{step:04d}/{total_step:04d}], Lr: {optimizer.param_groups[0]["lr"]}, Loss: {all_loss.data:.4f}, seg_loss: {seg_loss.data:.4f}, body_loss: {body_loss.data:.4f}')
            logging.info(f'TRAIN: Epoch [{epoch:03d}/{args.epoch:03d}], Step [{step:04d}/{total_step:04d}], Lr: {optimizer.param_groups[0]["lr"]}, Loss: {all_loss.data:.4f}, seg_loss: {seg_loss.data:.4f}, body_loss: {body_loss.data:.4f}')

        if step >=15000 and step % 200 == 0:
...
def val(test_loader, model, step, epoch, save_path, writer):
    ...
    with torch.no_grad():
        ...
        if mae < best_mae:
            ...
            #         # save
            #         save_video_path = os.path.join(save_path, 'figures')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--note',        type=str,   default='Note4/',                      help='the experiment note'                  )
